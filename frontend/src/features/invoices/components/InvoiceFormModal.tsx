...
export const InvoiceFormModal: React.FC<InvoiceFormModalProps> = ({ isOpen, onClose, onSubmit }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!customerId) {
        alert("Please specify a customer ID");
        return;
    }
  ...
  const draftVat = draftSubtotal * 0.11;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
      ...
      >
        <div className="flex justify-between items-center px-6 py-4 border-b border-slate-100 dark:border-slate-700">
          <h2 className="text-lg font-semibold dark:text-white">Create Draft Invoice</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 flex-1 overflow-y-auto">
          <div className="mb-6 max-w-sm">
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Customer Name / ID</label>
            <input
              type="text"
              required
              value={customerId}
          ...
          <div className="mb-4">
            <div className="flex justify-between items-center mb-2">
              <h3 className="text-sm font-medium text-slate-700 dark:text-slate-300">Invoice Items</h3>
              <button
                type="button"
